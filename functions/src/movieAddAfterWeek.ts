...
type theaterNames = 'ほげほげ映画館' | 'ぴよぴよ映画館' | 'ふがふが映画館'
...
export const FirestoreAddMovie = async () => {
  ...
  // 時間の数だけ回す
  const forDateTime = (theaterName: theaterNames) => {
  ...
  // 映画館の数だけ回す
  theaters.forEach(theater => forDateTime(theater as theaterNames))
}
